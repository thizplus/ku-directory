...
func (s *UserServiceImpl) Register(ctx context.Context, req *dto.CreateUserRequest) (*models.User, error) {
	...
	publicSlug := generatePublicSlug(req.FirstName, req.LastName)
...
func (s *UserServiceImpl) UpdateGeminiSettings(ctx context.Context, userID uuid.UUID, req *dto.GeminiSettingsRequest) (*models.User, error) {
	user, err := s.userRepo.GetByID(ctx, userID)
	if err != nil {
		return nil, errors.New("user not found")
	}

	user.GeminiAPIKey = req.APIKey
	user.GeminiModel = req.Model
	user.UpdatedAt = time.Now()
...
// generatePublicSlug creates a unique public slug from user's name
func generatePublicSlug(firstName, lastName string) string {
	base := strings.ToLower(firstName)
	if lastName != "" {
		base = base + "-" + strings.ToLower(lastName)
	}
